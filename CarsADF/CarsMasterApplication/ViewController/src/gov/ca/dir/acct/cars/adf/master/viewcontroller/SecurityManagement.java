...
public class SecurityManagement {
    ...
    public static List getAllUserRoles(String userName, String searchPath) {
        String methodName =
            Thread.currentThread().getStackTrace()[1].getMethodName();
        List returnList = new ArrayList();
        IdentityStoreFactory storeFactory =
            SecurityManagement.getIdentityStoreFactory();
        IdentityStore store = null;
     ...
     *  TODO: Commented out for now due to not being in use
     *          Consider refactoring if used to not show up on veracode 
    public static void resetPasswordForUser(String userId) {
        ...
        try {
            ...
            if (user != null) {
                ...
                //TODO: Veracode will treat this line as hardcoded password
...
*/
    ...
    private static final class IdentityStoreConfigurator {
        ...
        private static IdentityStore initializeDefaultStore() {
            ...
            try {
                store =
                        IdentityStoreConfigurator.idStoreFactory.getIdentityStoreInstance(storeEnv);
            } catch (IMException e) {
                OIDLogger.severe("Exception in " + methodName + " " +
                                 e.getMessage() + " ", e);
                // re throw exception here
