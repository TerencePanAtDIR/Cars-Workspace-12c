...
public class PurgeRunningFaultedSoaInstances {

    public Hashtable<String, String> getJNDIProps() {
        Hashtable<String, String> jndiProps = new Hashtable<String, String>();
        jndiProps.put(Context.PROVIDER_URL, "t3://cssoapy1:8001/soa-infra");
        jndiProps.put(Context.INITIAL_CONTEXT_FACTORY,
                      "weblogic.jndi.WLInitialContextFactory");
        jndiProps.put(Context.SECURITY_PRINCIPAL, "tpan");
        jndiProps.put(Context.SECURITY_CREDENTIALS, "Waymore2!");
    ...
    public static void main(String[] args) {
        ...
        Calendar beginDateCal = new GregorianCalendar(2019, Calendar.JUNE, 22);
        Date beginDate = beginDateCal.getTime();
        Calendar endDateCal = new GregorianCalendar(2019, Calendar.JUNE, 24);
        Date endDate = endDateCal.getTime();
        //instanceFilter.setMinCreationDate(arg0);
        //instanceFilter.setState();
        //instanceFilter.setECID("0000Mi2JTI05uX_U1QuXKV1T3S^q002dPO");
        instanceFilter.setMinCreationDate(beginDate);
        instanceFilter.setMaxCreationDate(endDate);
        instanceFilter.setState(3);
        try {
            ...
            for (CompositeInstance compositeInstance: composites){
                //System.out.println(compositeInstance.getExecutionState());
                if(compositeInstance.getExecutionState() == 0){
                    System.out.println("aborting ECID: " + compositeInstance.getECID());
                    System.out.println(compositeInstance.getConversationId());
                    List<ComponentInstance> componentInstances 
                        = compositeInstance.getChildComponentInstances(cif);
                    for(ComponentInstance componentInstance : componentInstances){
                        System.out.println(componentInstance.getComponentName());
                    }
